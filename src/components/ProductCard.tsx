    ...
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow group">
      ...
      <div className="p-4">
        <Link href={`/catalog/${product.id}`}>
          <h3 className="font-medium text-gray-800 text-sm leading-tight hover:text-rose-600 transition-colors line-clamp-2 mb-2">
            {product.name}
          </h3>
        </Link>

        {product.category_name && (
          <span className="text-xs text-rose-400 font-medium">{product.category_name}</span>
        )}
        ...
        <div className="flex items-center justify-between mt-3">
          ...
          <button
            ...
            className="bg-rose-500 hover:bg-rose-600 disabled:bg-gray-200 text-white p-2.5 rounded-xl transition-colors"
          >
            <ShoppingBag size={16} />
          </button>
        </div>
