...
export default async function ProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
      <Link href="/catalog" className="inline-flex items-center gap-2 text-gray-500 hover:text-green-600 text-sm mb-6 transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-14">
        ...
        <div className="flex flex-col">
          {product.category_name && (
            <Link href={`/catalog?category=${product.category_slug}`} className="text-green-400 text-sm font-medium hover:text-green-600 transition-colors mb-2">
              {product.category_name}
            </Link>
          )}
          ...
          <div className="mt-8 space-y-3 border-t border-gray-100 pt-6">
            <div className="flex items-center gap-3 text-sm text-gray-500">
              <Truck size={17} className="text-green-400 shrink-0" />
              <span>Доставка по городу за 2–3 часа</span>
            </div>
            <div className="flex items-center gap-3 text-sm text-gray-500">
              <Package size={17} className="text-green-400 shrink-0" />
            ...
            {related.map(p => (
              <Link key={p.id} href={`/catalog/${p.id}`} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="p-3">
                  ...
                  <p className="text-green-600 font-bold text-sm">{p.price.toLocaleString('ru-RU')} ₽</p>
                </div>
              </Link>
            ))}
          </div>
